 ...
 * @param imgSize ：图片大小
 * @param imgNum : 图片个数
 * @param statusId : 状态：0：添加 1：编辑
 * @param operatedName : 操作图片路径
 */
function uploadVideo(className, id, imgSize, imgNum, statusId, operatedName){
    ...
    $(className).on("click", '.content-img-list-item a .gcllajitong', function() {
        ...
        if($("#"+statusId).val() == 1){
            $("#"+operatedName).val($(this).parent().parent().prev()[0].currentSrc +","+ $("#"+operatedName).val());
        }
    ...
    $(className).on("click", '.content-img-list-item a .gclfangda', function() {
        $('.videos').html("<video id=\"video\" style='width:640px; height:360px;'  src='"+$(this).parent().parent().prev()[0].currentSrc+"' preload=\"auto\" controls=\"controls\" autoplay=\"autoplay\"></video>" +
            "<img onClick=\"close1()\" class=\"vclose\" src=\"images/gb.png\" width=\"25\" height=\"25\" style='background: #999'/>");
        $('.videos').show();
    });
    ...
    $('#'+ id).on('change', function(e) {
        ...
        if (imgName.length > imgNum){
            return alert("最多可以上传"+ imgName.length + "个视频");
        }

        for (var i = 0; i < fileList.length; i++) {
            var imgSrcI = getVideotURL(fileList[i]);
        ...
        if($("#"+statusId).val() == 1){
            editVideoContent(className, imgSrc);
        }else{
            addVideoContent(className, imgSrc);
        }
...
//添加图片展示
function addVideoContent(obj, imgSrc) {
    $(obj).html("");
    for (var a = 0; a < imgSrc.length; a++) {
        var oldBox = $(obj).html();
        $(obj).html(oldBox + '<li class="content-img-list-item"><video src="' + imgSrc[a] + '" alt="" style="width: 100%;height: 100%"></video>' +
...
function editVideoContent(obj, imgSrc) {
    var oldBox = $(obj).html();
    $(obj).html("");
    for (var a = 0; a < imgSrc.length; a++) {
        $(obj).html(oldBox + '<li class="content-img-list-item"><video src="' + imgSrc[a] + '" alt="" style="width: 100%;height: 100%"></video>' +
 ...
 * @param deleteUrl
 */
function handleVideo(originalUrl, deleteUrl) {
    var picArr = null;
