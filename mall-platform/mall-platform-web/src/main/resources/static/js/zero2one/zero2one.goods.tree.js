;(function($) {
        ...
        ZdCascader.prototype._construct = function() {
            var self = this;
            this.$container = this.$el.wrap(`<div class="${this.CLASS.wrap}"></div>`).wrap(`<div class="${this.CLASS.inputwrap}"></div>`).addClass(this.CLASS.input).prop('readonly', true).closest('.' + this.CLASS.wrap);
        ...
        ZdCascader.prototype._wrapClick = function() {
            ...
            this.$dropdownWrap.find(this.CLASS.menuList).eq(0).find('li.' + this.CLASS.checkClass.menuNodeSelected).addClass(this.CLASS.checkClass.nodeAnchor);
        };
        ZdCascader.prototype._nodeClick = function(event) {
            var $that = event.currentTarget ? $(event.currentTarget) : $(event);
        ...
        ZdCascader.prototype._loadChildren = function($parentNode) {
            ...
            $.each(data.children, (i,m)=>{
                    ...
                    var $label = $(`<span class="${this.CLASS.menuNodeLabel}">${m.name}</span>`);
                    ...
                    if (m.children && m.children.length > 0)
                        $li.append($icon);
                    else if (selectedId.indexOf(m.id) >= 0) {
                        this.$dropdownWrap.find('.' + this.CLASS.checkClass.nodeSelectedIcon).remove();
        ...
        ZdCascader.prototype.reload = function(data) {
            ...
            $.each(data, (i,m)=>{
                    ...
                    if (m.children && m.children.length > 0)
                        $li.append($icon);
                    else if (selectedData && m.id == selectedData.id) {
                        this.$dropdownWrap.find('.' + this.CLASS.checkClass.nodeSelectedIcon).remove();
            ...
            this.$dropdownWrap.find('li.' + this.CLASS.checkClass.nodeAnchor).removeClass(this.CLASS.checkClass.nodeAnchor);
            this.$dropdownWrap.append($firstWrap).find(this.CLASS.menuNode).eq(0).focus().addClass(this.CLASS.checkClass.nodeAnchor);
        };
        ...
        ZdCascader.prototype._movedown = function() {
            ...
            if ($selected.length === 0)
                $selected = this.$dropdownWrap.find('.' + this.CLASS.menuWrap).eq(0).find('.' + this.CLASS.menuNode).eq(0).addClass(this.CLASS.checkClass.nodeAnchor);
            else if ($selected.next().length > 0)
                $selected = $selected.removeClass(this.CLASS.checkClass.nodeAnchor).next().addClass(this.CLASS.checkClass.nodeAnchor);
        ...
        ZdCascader.prototype._moveup = function() {
            ...
            if ($selected.prev().length > 0)
                $selected = $selected.removeClass(this.CLASS.checkClass.nodeAnchor).prev().addClass(this.CLASS.checkClass.nodeAnchor);
            this._loadChildren($selected);
        };
        ZdCascader.prototype._moveleft = function() {
            ...
            $selected = $leftWrap.find('li.' + this.CLASS.checkClass.menuNodeSelected).length > 0 ? $leftWrap.find('li.' + this.CLASS.checkClass.menuNodeSelected).eq(0) : $leftWrap.find('li' + this.CLASS.menuNode).eq(0);
